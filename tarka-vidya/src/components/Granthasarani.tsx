...
import { GitFork, BookOpen, Compass, ArrowRight, CornerDownRight, HelpCircle, Activity, Award, User, Volume2, Pause, ShieldAlert, ChevronLeft, ChevronRight, Share2 } from "lucide-react";
...
export default function TraditionMap({
  ...
  // Academic Share Modal State
  const [shareModalPayload, setShareModalPayload] = useState<AcademicSharePayload | null>(null);
  const [isShareModalOpen, setIsShareModalOpen] = useState(false);

  const handleSharePadarthas = () => {
    setShareModalPayload({
      title: "Seven Ontological Categories (सप्तपदार्थाः)",
      sanskritText: "द्रव्य-गुण-कर्म-सामान्य-विशेष-समवाय-अभावाः सप्त पदार्थाः",
      transliteration: "dravya-guṇa-karma-sāmānya-viśeṣa-samavāya-abhāvāḥ sapta padārthāḥ",
      translation: "The 7 real categories of the universe according to Vaiśeṣika & Nyāya realism: Substance (Dravya), Quality (Guṇa), Action (Karma), Generality (Sāmānya), Particularity (Viśeṣa), Inherence (Samavāya), and Non-existence (Abhāva).",
      source: "Annaṁbhaṭṭa's Tarkasaṁgraha & Kaṇāda's Vaiśeṣikasūtra",
      category: "comparative-insight",
      url: typeof window !== "undefined" ? `${window.location.origin}${window.location.pathname}?tab=overview&tabView=padarthas` : "https://tarkavidya.in",
    });
    setIsShareModalOpen(true);
  };

  const handleSharePramanas = () => {
    setShareModalPayload({
      title: "Comparative Pramāṇa Matrix across Indian Philosophical Schools",
      sanskritText: "प्रत्यक्षानुमानोपमानशब्दाः प्रमाणानि",
      transliteration: "pratyakṣānumānopamānaśabdāḥ pramāṇāni",
      translation: "Epistemological distribution of valid knowledge instruments: Cārvāka (1), Buddhism/Vaiśeṣika (2), Sāṅkhya (3), Nyāya (4), Prabhākara Mīmāṃsā (5), Bhāṭṭa Mīmāṃsā & Advaita Vedānta (6).",
      source: "Tarka-Vidyā Comparative Epistemological Matrix",
      category: "comparative-insight",
      url: typeof window !== "undefined" ? `${window.location.origin}${window.location.pathname}?tab=overview&tabView=pramanas` : "https://tarkavidya.in",
    });
    setIsShareModalOpen(true);
  };

  const handleShareFallacies = () => {
    setShareModalPayload({
      title: "Five Syllogistic Fallacies (पञ्चहेत्वाभासाः)",
      sanskritText: "सव्यभिचार-विरुद्ध-सत्प्रतिपक्ष-असिद्ध-बाधिताः पञ्च हेत्वाभासाः",
      transliteration: "savyabhicāra-viruddha-satpratipakṣa-asiddha-bādhitāḥ pañca hetvābhāsāḥ",
      translation: "The 5 structural defects of logical inference in Nyāya: Irregular reason (Savyabhicāra), Contradictory reason (Viruddha), Counterbalanced reason (Satpratipakṣa), Unproven reason (Asiddha), and Sublated reason (Bādhita).",
      source: "Akṣapāda Gautama's Nyāyasūtra & Tarkasaṁgraha",
      category: "comparative-insight",
      url: typeof window !== "undefined" ? `${window.location.origin}${window.location.pathname}?tab=overview&tabView=fallacies` : "https://tarkavidya.in",
    });
    setIsShareModalOpen(true);
  };

  const handleShareSchool = (schoolKey: string) => {
    const node = (nodes as any)[schoolKey];
    if (!node) return;
    setShareModalPayload({
      title: `${node.englishTitle} (${node.iastTitle})`,
      sanskritText: transliterate(node.title, targetScript),
      transliteration: node.iastTitle,
      translation: `${node.description} Foundational Ācārya: ${node.founder || "Classical Tradition"}. Accepted Pramāṇas: ${node.acceptedPramanas || "Various"}.`,
      source: `Tarka-Vidyā Indian Philosophical Systems Matrix`,
      category: "comparative-insight",
      url: typeof window !== "undefined" ? `${window.location.origin}${window.location.pathname}?tab=overview&school=${schoolKey}` : "https://tarkavidya.in",
    });
    setIsShareModalOpen(true);
  };

  const handleShareDialectic = () => {
    setShareModalPayload({
      title: "The Dialectical Structure of Tarka (तर्कस्वरूपविमर्शः)",
      sanskritText: "तर्कः खलु न प्रमाणसङ्गृहीतो न प्रमाणान्तरं प्रमाणानुकूलस्तत्त्वज्ञानाय कल्पते ।",
      transliteration: "tarkaḥ khalu na pramāṇasaṅgṛhīto na pramāṇāntaraṁ pramāṇānukūlas tattvajñānāya kalpate",
      translation: "Tarka is neither subsumed within the instruments of knowledge nor is it an independent pramāṇa; rather, it is supportive conditional reasoning guarding the instruments of knowledge toward truth-determination.",
      source: "Vātsyāyana Bhāṣya on Nyāyasūtra 1.1.1 & Dasgupta's History of Indian Philosophy",
      category: "comparative-insight",
      url: typeof window !== "undefined" ? `${window.location.origin}${window.location.pathname}?tab=overview` : "https://tarkavidya.in",
    });
    setIsShareModalOpen(true);
  };
    ...
    <div className="space-y-8 animate-fade-in font-sans" id="tradition-map-portal">
      ...
      <div className="bg-white border-2 border-[#1A1A1A] p-6 lg:p-8 rounded-none shadow-none space-y-6 manuscript-margin-line pl-6 md:pl-10 academic-blueprint-grid classy-transition">
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b-2 border-[#1A1A1A] pb-4">
            ...
            {/* Audio Chanting Demo Player & Share */}
            <div className="shrink-0 flex items-center gap-2">
              ...
              <button
                onClick={handleShareDialectic}
                className="flex items-center gap-1.5 text-xs text-[#8C6239] px-3.5 py-2 font-bold uppercase tracking-widest border-2 border-[#8C6239] bg-white hover:bg-[#8C6239] hover:text-white rounded-none transition-all cursor-pointer shadow-xs active:scale-95"
                title="Share this Dialectics discourse insight"
              >
            ...
            <div className="bg-white border-2 border-[#1A1A1A] p-5 rounded-none shadow-none space-y-4">
              <div className="border-b border-stone-200 pb-3 flex items-center justify-between gap-2">
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  <button
                    onClick={() => handleShareSchool(selectedNode)}
                    className="p-1.5 border-2 border-[#8C6239] bg-white hover:bg-[#8C6239] hover:text-white text-[#8C6239] transition-all cursor-pointer text-xs font-bold flex items-center gap-1"
                    title={`Share ${nodes[selectedNode as keyof typeof nodes]?.englishTitle || "School"} on WhatsApp, X, or Facebook`}
                  >
                    <Share2 className="w-3.5 h-3.5" />
                    <span className="hidden sm:inline text-[9px] uppercase">Share</span>
                  </button>

                  <div className="flex items-center gap-1 bg-[#F5F2EA] p-1 border-2 border-[#1A1A1A]">
                    <button 
      ...
      <div className="bg-white border-2 border-[#1A1A1A] rounded-none p-6 shadow-none mt-8 space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b-2 border-[#1A1A1A] pb-3 gap-4">
          ...
          {/* Tabs switch & Share Button */}
          <div className="flex flex-wrap items-center gap-2 self-start sm:self-center">
            <button
              onClick={() => {
                if (overviewTab === "padarthas") handleSharePadarthas();
                else if (overviewTab === "pramanas") handleSharePramanas();
                else if (overviewTab === "fallacies") handleShareFallacies();
                else handleShareDialectic();
              }}
              className="px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-none transition-all cursor-pointer bg-[#FFF9E6] hover:bg-[#8C6239] hover:text-white text-[#8C6239] border-2 border-[#8C6239] flex items-center gap-1.5 shadow-xs"
              title="Share the current comparative epistemological matrix on WhatsApp, X, Facebook, or Copy Link"
            >
              <Share2 className="w-3.5 h-3.5" />
              <span>Share Matrix</span>
            </button>

            <div className="flex flex-wrap bg-white p-1 rounded-none border-2 border-[#1A1A1A]">
              <button
